...
void printBits(uint64 n);
uint64 generateKey(int key_size);
uint64 getBit(uint64 number, int bitIdx);
uint64 permute(uint64 key, int* table, int length);
void splitKey(uint64 key, uint32* C, uint32* D, int size);
uint64 shiftKeys(uint64 value, int shifts);
void createSubkeys(uint64 key, uint64* subKeys);
uint64 encryptMessage(uint64 key, uint64 message);
uint32 func(uint32 data, uint64 key);
...
uint64 encryptMessage(uint64 message, uint64 key)
{	
	uint64 K[16];
	createSubkeys(key, K);
